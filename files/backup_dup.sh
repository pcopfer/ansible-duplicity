...
do
BN=`basename $line`
duplicity incremental --full-if-older-than 1M $line sftp://$BACKUP_CREDENTIALS/$HN/$BN
rc=$?
duplicity remove-all-inc-of-but-n-full 2 --force sftp://$BACKUP_CREDENTIALS/$HN/$BN
duplicity remove-all-but-n-full 4 --force sftp://$BACKUP_CREDENTIALS/$HN/$BN
done
unset PASSPHRASE
exit $rc
